...
run week3_exercise2.py MM9_TSS_CHR6_-5000_+5000.gff MES_H3K4ME3_chr6_peaks.bed chr6_H3K4Me3_genes.txt
'''
...
def main():
    ...
    for line in tssTable:
        ...
        #4. check each TSS for overlap w/ the enriched regions
        '''
        for regionLine in regionTable:
            regionChrom = regionLine[0]
            regionStart = int(regionLine[1])
            regionEnd = int(regionLine[2])

            #if the tss overlaps the enriched region
            if testOverlap(tssChrom,tssStart,tssEnd,regionChrom,regionStart,regionEnd):
                #5. Store the names of genes whose TSSs overlap the enriched regions in the list created in #3                        
    ...
    print "Identified %s genes that are overlapped by the enriched region" % (len(overlappingGenesTable))
